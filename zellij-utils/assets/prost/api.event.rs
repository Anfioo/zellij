...
/// plugin_command.PaneId 的副本，因为 protobuf 不支持递归导入
#[allow(clippy::derive_partial_eq_without_eq)]
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct PaneId {
...
/// `LayoutParsingError` 中的嵌套消息和枚举类型。
...
/// `PaneScrollbackResponse` 中的嵌套消息和枚举类型。
...
pub enum EventType {
    ...
    /// / 来自插件某个工作线程的消息
    CustomMessage = 10,
    /// / 在 Zellij CWD 文件夹中的某处创建了文件
    FileSystemCreate = 11,
    /// / 在 Zellij CWD 文件夹中的某处访问了文件
    FileSystemRead = 12,
    /// / 在 Zellij CWD 文件夹中的某处修改了文件
    FileSystemUpdate = 13,
    /// / 在 Zellij CWD 文件夹中的某处删除了文件
    FileSystemDelete = 14,
...
/// plugin_command.PaneType 的副本，因为 protobuf 不支持递归导入
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, PartialOrd, Ord, ::prost::Enumeration)]
#[repr(i32)]
pub enum PaneType {
    Terminal = 0,
