...
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct Position {
    /// 从 uint32 改为 int32 以支持负行号
    #[prost(int32, tag="1")]
    pub line: i32,
    /// 从 uint32 改为 uint64 以支持大列号
    #[prost(uint64, tag="2")]
    pub column: u64,
}
#[allow(clippy::derive_partial_eq_without_eq)]
...
#[derive(Clone, PartialEq, ::prost::Oneof)]
    pub enum PlacementType {
        ...
        /// 这些新变体使我们能够在发送数据的同时保留以前的变体，而不会通过更改上述类型来损害向后兼容性
        #[prost(message, tag="6")]
...
/// `FloatingCoordinate` 中的嵌套消息和枚举类型。
...
pub struct RunCommandAction {
    ...
    /// 添加了缺失的 originating_plugin 字段
    #[prost(message, optional, tag="7")]
    pub originating_plugin: ::core::option::Option<OriginatingPlugin>,
    /// 添加了缺失的 use_terminal_title 字段
    #[prost(bool, tag="8")]
...
pub struct TiledPaneLayout {
    ...
    /// 注意：run_instructions_to_ignore 未在此处表示，因为它是仅在服务端内部使用的字段，不属于服务端/客户端契约的一部分
...
pub struct LayoutConstraintWithValue {
    ...
    /// 仅用于 MAX_PANES、MIN_PANES、EXACT_PANES
    #[prost(uint32, optional, tag="2")]
...
/// `RunPluginLocationData` 中的嵌套消息和枚举类型。
...
pub struct OriginatingPlugin {
    #[prost(uint32, tag="1")]
    pub plugin_id: u32,
    /// ClientId 为 u16，但为了 protobuf 兼容性使用 uint32
    #[prost(uint32, tag="2")]
    pub client_id: u32,
    /// Context 为 BTreeMap<String, String>
    #[prost(map="string, string", tag="3")]
...
// 注意：旧的 MousePressEvent、MouseReleaseEvent、MouseHoldEvent、MouseScrollEvent、
// MouseButton 和 ScrollDirection 已移除——替换为统一的 MouseEvent 结构

#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, PartialOrd, Ord, ::prost::Enumeration)]
#[repr(i32)]
pub enum SearchDirection {
    Unspecified = 0,
    /// 从 FORWARD 更改以匹配 Rust 枚举
    Down = 1,
    /// 从 BACKWARD 更改以匹配 Rust 枚举
    Up = 2,
}
...
pub enum SearchOption {
    Unspecified = 0,
    /// 从 CASE_SENSITIVE 更改以匹配 Rust 枚举
    CaseSensitivity = 1,
    /// 从 WHOLE_WORDS 更改以匹配 Rust 枚举
    WholeWord = 2,
    /// 未更改——已匹配
    Wrap = 3,
}
...
/// 空消息 (just indicates connection status request)
#[allow(clippy::derive_partial_eq_without_eq)]
#[derive(Clone, PartialEq, ::prost::Message)]
