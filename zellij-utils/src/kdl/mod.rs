        ...
        match action_name {
            ...
            "MessagePlugin" => {
                ...
                let name = name
                    ...
                    // 最后使用 uuid，至少让这条消息有某种标识符
                    .or_else(|| Some(Uuid::new_v4().to_string()));

                Ok(Action::KeybindPipe {
                    name,
                    payload,
                    args: None, // TODO: 如有需要，考虑支持这一点
                    plugin: plugin_path,
                    configuration,
                    launch_new,
                    skip_cache,
                    floating: Some(should_float),
                    in_place: None, // TODO: 支持这一点
                    cwd,
                    pane_title: title,
                    plugin_id: None,
                })
            ...
            "MessagePluginId" => {
                ...
                let name = name
                    // 如果未提供名称，我们使用 uuid，至少让这条消息有某种标识符
                    .or_else(|| Some(Uuid::new_v4().to_string()));
...
impl Keybinds {
    ...
    // 为序列化尽量减少键位条目，这样重复条目会出现在
    // 后面的 "shared" 节点中，而不是每个模式各出现一次
    fn minimize_entries(
        &self,
    ...
    pub fn to_kdl(&self, should_clear_defaults: bool) -> KdlNode {
        ...
        // 我们显式地这样做，以便配置中模式的排序顺序更易
        // 阅读——这实际上比在这种情况下实现 Ord 更简洁（也更清晰）
...
impl Config {
    pub fn from_kdl(kdl_config: &str, base_config: Option<Config>) -> Result<Config, ConfigError> {
        ...
        // TODO：处理存在多个此类块的情况（例如两个 "keybinds"），
        // 此时应给出信息丰富的解析错误
...
impl PaneInfo {
    pub fn decode_from_kdl(kdl_document: &KdlDocument) -> Result<(usize, Self), String> {
        ...
        let pane_info = PaneInfo {
            ...
            plugin_url,
            is_selectable,
            index_in_pane_group: Default::default(), // 我们不序列化此项
            default_fg: None,
            default_bg: None,
        };
...
fn can_bind_theme_actions() {
    // 针对 https://github.com/zellij-org/zellij/issues/5297 的回归测试
    // SetDarkTheme / SetLightTheme / ToggleTheme 可通过 CLI 使用，但过去会被
    // 键位解析器以 "Unsupported action" 拒绝。
    let fake_config = r#"
        keybinds {
            normal {
