...
impl<'a> MainScreen<'a> {
    ...
    const HELP_TEXT_WITH_CLICK: &'static str = "帮助: 点击或 Shift-点击 在浏览器中打开";
    const HELP_TEXT_SHIFT_ONLY: &'static str = "帮助: Shift-点击 在浏览器中打开";
    pub fn new(
        token_list_is_empty: bool,
        web_server_started: bool,
        web_server_error: &'a Option<String>,
    ...
    fn render_link_help(&self, x: usize, y: usize) {
        let help_text = if self.link_executable.is_some() {
            ...
            Text::new(Self::HELP_TEXT_SHIFT_ONLY).color_range(3, 4..=11)
        };
