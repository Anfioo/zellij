...
pub fn compact_layout_full(help: &ModeInfo) -> LinePart {
    ...
    let mut bits = vec![
        Style::new().paint(" 提示： "),
        Style::new().paint("界面占用太多空间？使用以下方式启动 Zellij："),
        Style::new()
            .fg(green_color)
            .bold()
            .paint("zellij -l compact"),
        Style::new().paint(" 或使用以下键位移除窗格边框："),
    ];
...
pub fn compact_layout_medium(help: &ModeInfo) -> LinePart {
    ...
    let mut bits = vec![
        Style::new().paint(" 提示： "),
        Style::new().paint("为节省屏幕空间，使用以下方式启动 Zellij："),
        Style::new()
            .fg(green_color)
            .bold()
            .paint("zellij -l compact"),
        Style::new().paint(" 或使用以下键位移除边框："),
    ];
    bits.extend(add_keybinds(help));
    strings!(&bits)
}
...
pub fn compact_layout_short(help: &ModeInfo) -> LinePart {
    ...
    let mut bits = vec![
        Style::new().paint(" 节省屏幕空间，使用以下方式启动："),
        Style::new()
            .fg(green_color)
            .bold()
            .paint("zellij -l compact"),
