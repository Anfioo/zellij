...
pub fn move_focus_hjkl_tab_switch_full(help: &ModeInfo) -> LinePart {
    ...
    let mut bits = vec![
        Style::new().paint(" 提示："),
        Style::new().paint("用 切换焦点时"),
    ];
    bits.extend(add_keybinds(help));
    bits.push(Style::new().paint("，移出屏幕左/右边缘将聚焦下一个标签页。"));
    strings!(&bits)
}
...
pub fn move_focus_hjkl_tab_switch_medium(help: &ModeInfo) -> LinePart {
    ...
    let mut bits = vec![
        Style::new().paint(" 提示："),
        Style::new().paint("用 切换焦点"),
    ];
    bits.extend(add_keybinds(help));
    bits.push(Style::new().paint(" 移出屏幕将聚焦下一个标签页。"));
    strings!(&bits)
}
...
pub fn move_focus_hjkl_tab_switch_short(help: &ModeInfo) -> LinePart {
    ...
    bits.push(Style::new().paint(" 移出屏幕边缘将聚焦下一个标签页。"));
    strings!(&bits)
}
