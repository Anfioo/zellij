...
fn get_keys_and_hints(mi: &ModeInfo) -> Vec<(String, String, Vec<KeyWithModifier>)> {
    ...
    if mi.mode == IM::Pane { vec![
        ...
        (s("向右拆分"), s("右"), single_action_key(&km, &[A::NewPane{direction: Some(Direction::Right), pane_name: None, start_suppressed: false}, TO_NORMAL])),
        (s("向下拆分"), s("下"), single_action_key(&km, &[A::NewPane{direction: Some(Direction::Down), pane_name: None, start_suppressed: false}, TO_NORMAL])),
        ...
        (s("将窗格移到新标签页"), s("移出"), single_action_key(&km, &[A::BreakPane, TO_NORMAL])),
