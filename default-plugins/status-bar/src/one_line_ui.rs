...
fn add_keygroup_separator(help: &ModeInfo, max_len: usize) -> Option<LinePart> {
    ...
    if help.session_dimmed.unwrap_or(false) {
        ...
        let mode_help_text = match help.mode {
            InputMode::RenamePane => Some("重命名窗格"),
            InputMode::RenameTab => Some("重命名标签页"),
            InputMode::EnterSearch => Some("输入搜索词"),
            InputMode::Search => Some("搜索中"),
            _ => None,
        };
...
fn get_keys_and_hints(mi: &ModeInfo) -> Vec<(String, String, Vec<KeyWithModifier>)> {
    ...
    if mi.mode == IM::Pane { vec![
        ...
        (s("切换嵌入"), s("嵌入"), single_action_key(&km, &[A::TogglePaneEmbedOrFloating, TO_NORMAL])),
        (s("右拆分"), s("右"), single_action_key(&km, &[A::NewPane{direction: Some(Direction::Right), pane_name: None, start_suppressed: false}, TO_NORMAL])),
        (s("下拆分"), s("下"), single_action_key(&km, &[A::NewPane{direction: Some(Direction::Down), pane_name: None, start_suppressed: false}, TO_NORMAL])),
        (s("堆叠"), s("堆叠"), single_action_key(&km, &[A::NewStackedPane{command: None, pane_name: None, near_current_pane: false, no_focus: false, tab_id: None}, TO_NORMAL])),
        ...
        (s("将窗格拆为新标签"), s("拆出"), single_action_key(&km, &[A::BreakPane, TO_NORMAL])),
        (s("向左/右拆窗格"), s("拆分"), action_key_group(&km, &[
        ...
        (s("会话管理器"), s("管理器"), session_manager_key(&km)),
        ...
        (s("移动焦点"), s("移动"), action_key_group(&km, &[
            ...
            &[A::MoveFocus{direction: Dir::Up}], &[A::MoveFocus{direction: Dir::Right}]])),
        (s("下拆分"), s("下"), action_key(&km, &[A::NewPane{direction: Some(Dir::Down), pane_name: None, start_suppressed: false}, TO_NORMAL])),
        (s("右拆分"), s("右"), action_key(&km, &[A::NewPane{direction: Some(Dir::Right), pane_name: None, start_suppressed: false}, TO_NORMAL])),
        (s("全屏"), s("全屏"), action_key(&km, &[A::ToggleFocusFullscreen, TO_NORMAL])),
        (s("新标签"), s("新建"), action_key(&km, &[A::NewTab{
            tiled_layout: None,
            floating_layouts: vec![],
            swap_tiled_layouts: None,
            swap_floating_layouts: None,
        ...
        (s("上一标签"), s("上一个"), action_key(&km, &[A::GoToPreviousTab, TO_NORMAL])),
        (s("下一标签"), s("下一个"), action_key(&km, &[A::GoToNextTab, TO_NORMAL])),
        (s("选择窗格"), s("选择"), to_basemode_key),
